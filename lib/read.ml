...
module Make (M : Monad.T) = struct
  ...
  type ('a, 'b) monad_gen = 'b -> string -> int -> colors -> ('a * 'b * int) M.m
  type 'a monad = ('a, unit) monad_gen

  let lift (m : 'a M.m) : ('a, 'b) monad_gen =
    fun g _str index _colors ->
      let+ x = m in
      M.return (x, g, index)

  let return (type g) (type t) (x : t) : (t, g) monad_gen =
    fun g _str index _colors -> M.return (x, g, index)
  ...
  let num_colors (type g) : (int, g) monad_gen =
    fun g _str index colors ->
      M.return (Array.length colors, g, index)
